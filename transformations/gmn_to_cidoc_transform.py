...
# Getty AAT URI for transfer of rights
...
# Getty AAT URI for correspondance
...
def transform_p70_18_documents_disputing_party(data):
    ...
    P70_documents > E13_Attribute_Assignment > P140_assigned_attribute_to > E39_Actor
    ...
    # Check if attribute assignment already exists
    existing_assignment = None
    if 'cidoc:P70_documents' in data and len(data['cidoc:P70_documents']) > 0:
        # Use existing attribute assignment
        existing_assignment = data['cidoc:P70_documents'][0]
    else:
        # Create new attribute assignment
        assignment_uri = f"{subject_uri}/arbitration_assignment"
        existing_assignment = {
            '@id': assignment_uri,
            '@type': 'cidoc:E13_Attribute_Assignment'
        }
        data['cidoc:P70_documents'] = [existing_assignment]
    
    # Initialize P140 if it doesn't exist
    if 'cidoc:P140_assigned_attribute_to' not in existing_assignment:
        existing_assignment['cidoc:P140_assigned_attribute_to'] = []
    ...
    for party_obj in parties:
        ...
        # Add to attribute assignment
        existing_assignment['cidoc:P140_assigned_attribute_to'].append(party_data)
...
def transform_p70_19_documents_arbitrator(data):
    ...
    P70_documents > E13_Attribute_Assignment > P14_carried_out_by > E39_Actor
    ...
    # Initialize P14 if it doesn't exist
    if 'cidoc:P14_carried_out_by' not in existing_assignment:
        existing_assignment['cidoc:P14_carried_out_by'] = []
    ...
    for arbitrator_obj in arbitrators:
        ...
        # Add to attribute assignment
        existing_assignment['cidoc:P14_carried_out_by'].append(arbitrator_data)
...
def transform_p70_20_documents_dispute_subject(data):
    ...
    This creates a direct relationship between the document and the subject of the dispute.
    ...
    for subject_obj in subjects:
        ...
        # Add to P67
        data['cidoc:P67_refers_to'].append(subject_data)
...
def transform_p70_21_indicates_conceding_party(data):
    ...
    # Check if cession activity already exists
    existing_activity = None
    if 'cidoc:P70_documents' in data and len(data['cidoc:P70_documents']) > 0:
        # Use existing cession activity
...
def transform_p70_23_indicates_object_of_cession(data):
    ...
    for right_obj in rights:
        ...
        # Add to cession activity
        existing_activity['cidoc:P16_used_specific_object'].append(right_data)
...
def transform_export(input_file, output_file):
    """
    Transform an entire JSON-LD export file.
    ...
    try:
        ...
        if isinstance(data, list):
            ...
            # Handle JSON-LD with @graph
            data['@graph'] = [transform_item(item) for item in data['@graph']]
            transformed = data
        else:
...
def main():
    """Main entry point for command-line usage."""
    if len(sys.argv) != 3:
        print("Usage: python transform_gmn.py <input_file.json> <output_file.json>")
        print("\nExample:")
        print("  python transform_gmn.py omeka_export.json cidoc_compliant.json")
        sys.exit(1)
    
    input_file = sys.argv[1]
    output_file = sys.argv[2]
    
    success = transform_export(input_file, output_file)
    sys.exit(0 if success else 1)
...
def transform_item(item, include_internal=False):
    ...
    item = transform_p70_25_indicates_recipient(item)
    item = transform_p70_26_indicates_declaration_subject(item)
    item = transform_p138i_1_has_representation(item)
    item = transform_p1_4_has_loconym(item)
    item = transform_p11i_1_earliest_attestation_date(item)
    item = transform_p11i_2_latest_attestation_date(item)
